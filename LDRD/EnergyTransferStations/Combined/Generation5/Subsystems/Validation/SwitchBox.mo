...
model SwitchBox "Validation of flow switch box"
  ...
  Buildings.Fluid.Delays.DelayFirstOrder volRet(
    ...
    nPorts=3) "Mixing volume return" annotation (Placement(transformation(
        ...
        rotation=90,
        origin={70,0})));
  LDRD.EnergyTransferStations.BaseClasses.Pump_m_flow pum1(redeclare package
      Medium = Medium, final m_flow_nominal=m_flow_nominal)
    "Chilled water HX secondary pump"
    annotation (Placement(transformation(extent={{10,-10},{-10,10}})));
  LDRD.EnergyTransferStations.BaseClasses.Pump_m_flow pum2(redeclare package
      Medium = Medium, final m_flow_nominal=m_flow_nominal)
    "Chilled water HX secondary pump"
    annotation (Placement(transformation(extent={{-10,50},{10,70}})));
  LDRD.EnergyTransferStations.Combined.Generation5.Subsystems.SwitchBox floSwiBox(
    final m_flow_nominal=m_flow_nominal,
    redeclare final package Medium = Medium,
  ...
  Buildings.Fluid.Sources.Boundary_pT disWatBou(redeclare package Medium =
        Medium, nPorts=2) "District water boundary conditions" annotation (

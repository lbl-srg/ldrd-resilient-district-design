within LDRD.EnergyTransferStations.Combined.Generation5.Subsystems.Validation;
model Borefield
  ...
  final parameter Modelica.SIunits.Distance cooBor[nBorHol,2]=.LDRD.EnergyTransferStations.BaseClasses.computeCoordinates(
    nBorHol,
    dxy)
  ...
  Buildings.Fluid.Sources.Boundary_pT conWat(
    ...
    nPorts=2) "Condenser water boundary conditions" annotation (Placement(
        ...
        rotation=0,
        origin={-40,-2})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTInl(redeclare final package
      Medium = Medium, m_flow_nominal=borFie.pum.m_flow_nominal)
    "Water inlet temperature" annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={-10,20})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTOut(redeclare final package
      Medium = Medium, m_flow_nominal=borFie.pum.m_flow_nominal)
    "Water outlet temperature" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-10,-20})));
  parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Example datBorFie(conDat=
        Buildings.Fluid.Geothermal.Borefields.Data.Configuration.Example(cooBor=
         cooBor, dp_nominal=0)) "Borefield design data"

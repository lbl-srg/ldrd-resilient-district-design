...
model SwitchBox
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput mPos_flow(final unit="kg/s")
    ...
    iconTransformation(extent=
           {{-140,20},{-100,60}})));
  // COMPONENTS
  LDRD.EnergyTransferStations.BaseClasses.Junction splSup(redeclare final
      package Medium = Medium, m_flow_nominal={1,1,1}*m_flow_nominal)
    "Flow splitter" annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=-90,
        origin={-20,40})));
  LDRD.EnergyTransferStations.BaseClasses.Junction splRet(redeclare final
      package Medium = Medium, m_flow_nominal={1,1,1}*m_flow_nominal)
    "Flow splitter" annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={20,0})));
  Controls.SwitchBox con(final trueHoldDuration=trueHoldDuration, final
      falseHoldDuration=falseHoldDuration) "Switch box controller"

...
block CoolingTowersIntegrated "Cooling towers controller"
  ...
  Buildings.Experimental.DHC.EnergyTransferStations.Combined.Generation5.Controls.PIDWithEnable
    ...
    k=0.1,
    Ti=120,
    reverseActing=false) "Pump controller"
    annotation (Placement(transformation(extent={{-130,110},{-110,130}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant dTAppNom(final k=
        dTApp_nominal) "Design approach"
...
equation
  ...
  connect(zer.y, swiOff2.u3) annotation (Line(points={{122,80},{188,80},{188,92}},
                     color={0,0,127}));
  connect(comTLvgPre.y, mulAnd.u[3]) annotation (Line(points={{-38,0},{-30,0},{
  ...
  connect(one.y, comFanSig.f2) annotation (Line(points={{132,40},{136,40},{136,-48},
          {148,-48}},          color={0,0,127}));
  ...
  connect(delT1.y, conPID.u_m) annotation (Line(points={{-128,-100},{-120,-100},
          {-120,108}}, color={0,0,127}));
  connect(actSta.active, conPID.uEna) annotation (Line(points={{50,29},{50,20},{
          -124,20},{-124,108}}, color={255,0,255}));
  connect(dTAppNom.y, conPID.u_s)
    annotation (Line(points={{-148,120},{-132,120}}, color={0,0,127}));
  connect(zer.y, comPumSig.x1) annotation (Line(points={{122,80},{130,80},{130,128},
          {138,128}}, color={0,0,127}));
  connect(one.y, comPumSig.x2) annotation (Line(points={{132,40},{136,40},{136,116},
          {138,116}}, color={0,0,127}));
  connect(m_flow, comPumSig.f1) annotation (Line(points={{-200,100},{120,100},{120,
          124},{138,124},{138,124}}, color={0,0,127}));
  connect(conPID.y, comPumSig.u)
    annotation (Line(points={{-108,120},{138,120}}, color={0,0,127}));
  connect(masFlowNom.y, comPumSig.f2) annotation (Line(points={{82,80},{90,80},{
          90,112},{138,112}}, color={0,0,127}));
  connect(comPumSig.y, swiOff2.u1) annotation (Line(points={{162,120},{180,120},
